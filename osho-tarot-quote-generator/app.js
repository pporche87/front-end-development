...
function updateCard() {
    do {
        var index = Math.floor(Math.random() * majorarcana.length);
    } while(currentIndex === index);
    
    displayedCard = majorarcana[index];
    currentIndex = index;
    
    $("#cardnum").html(majorarcana[index].number);
    $("#cardname").html(majorarcana[index].card);
    $('#image').html($('<img>',{id:"image",src: majorarcana[index].url}))
    $("#commentary").html(majorarcana[index].commentary);
    $("#share-quote").on("click", function(even){
       event.preventDefault();
       window.open("https://twitter.com/intent/tweet?text=" + encodeURIComponent(majorarcana[index].commentary));
    });
}

$(function(){
   updateCard();     
   $("#generator").click(updateCard);
});
